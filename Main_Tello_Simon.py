...
class TelloControl:
    ...
    def __init__(self, dirpath=None, debug=True):
        ...
        self.searchingstep = 30  # in centimeters ' how far to go to each side in searching for screen square
        ...
        self.takeoffaddvalue = 140  # how many centimeters to add to tello in order to be on screen height
            ...
            while True:  # needs to get the same data couple of times
                    ...
                    if success:
                        ...
                        if same_data < self.same_data_times:
                            return True, final_data, moved_close
                    datalist.append(data)
    ...
    def SendDataToRobot(self, data):
        if self.debug:
            ...
            logging.info(f"sending data to a robot {data}")
        return True
    ...
    def FixPositionByScreenBB(self):
        ...
        if displacement_x <= 0:
            ...
            self.TelloMove("left", int(abs(displacement_x)*self.movementscale))
        else:
            if self.debug:
                print(f"moving right")
                logging.info(f"moving right")
            self.TelloMove("right", int(abs(displacement_x) * self.movementscale))
...
def main():
    ...
    full_ditpath = r"C:\Users\davidra\Desktop\NINJA_RAFAEL\Ninja_Symon2\first_try20"
    ...
    while True:
        ...
        if MainTello.status == "sending data to a robot":
            time.sleep(5)  # wait and take another picture to ensure silver screen statue
